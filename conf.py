...
#
# The SAF documentation is embedded in the sources as originally made possible by Robb
# Matzke's `mkdoc` tool. That means the .rst files don't actually exist in the repo and
# are instead generated using mkdoc. The logic here mimics logic in SAF's Makefiles to
# implement 'make refman.rest' by listing the source files and then issuing calls to
# mkdoc to produce the .rst files from the sources. In addition, we need to generate
# several of SSlib data types used using the `sstypegen` tool. We run that first
...
f = open('index.rst','w')
f.write('SAF Software\n')
f.write('############\n')
f.write('SAF Software is `available on github <https://github.com/markcmiller86/SAF>`_\n\n')
...
htmlhelp_basename = 'SAFUseManual'
...
latex_documents = [
    (master_doc, 'SAFUseManual.tex', u'SAF User Manual',
     u'LLNL', 'manual'),
]
...
man_pages = [
    (master_doc, 'foo', u'SAF User Manual Documentation',
     [author], 1)
]
