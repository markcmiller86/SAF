...
main(int argc, char *argv[])
{
    ...
    SAF_TRY_BEGIN {
        strcpy(buffer, "NO VERSION");
        result = saf_version_string(1, buffer, sizeof buffer);
        if (result==buffer) {
            PASSED;
        } else {
            FAILED;
    ...
    if (rank==0)
       printf("This is SAF %s\n", buffer);

    {
#if 0 /* Why is this here? --rpm 2002-05-14 */
    SAF_ENTER(saf_final,1);
#endif
    }
    saf_final();
