 ...
 * Audience:    Public
 * Chapter:    	Tests
 * Purpose:    	Test of read of a triangle mesh with an identified boundary
 *              and fields defined on them.
 *
 * Description: This test demonstrates reading of a simple 2D triangular mesh
 *              immersed in 2D Euclidean vector space.  The boundary of the
 *              mesh is an identified subset consisting of the edges of some of
 *              these triangular elements.  The mesh has 2 fields defined on
 *              it--the global coordinate field and a rank 2 symmetric tensor.
 *              A scalar field is defined on only the boundary.  The global
 *              coordinate and scalar fields have their dofs defined on nodes
 *              while the tensor field's dofs are defined on the elements.  The
 *              program accepts one optional argument, do_interactive.  If the
 *              program is invoked without this argument, then it will attempt
 *              to read a mesh with 1 edge in x and 2 in y.  If it is invoked
 *              with do_interactive, the user will be prompted for the number
 *              of x and y edges.
 *
...
SAF_Cat *nodes = NULL,    /* Handles to the node, elements and edges */
        *elems = NULL,    /* categories. */
        *edges = NULL;
SAF_Set *mesh = NULL,     /* Handles to the mesh and boundary sets. */
        *boundary = NULL;
int edge_ct_x, edge_ct_y, /* The number of edges in the x and y directions. */
    ...
    edge_ct,              /* Total number of edges in the boundary. */
    failCount = 0;
...
{
   /* Fills the boundary connectivity, boundary node to global node, edge to
    * element and edge to element edge maps.
    */
   ...
   /* Left edge of mesh. */
   for (i = 0; i < edge_ct_y; i++)
   {
      node = NODE_ID(0, i);
      bndy_conn[conn_idx++] = node;
      bndy_conn[conn_idx++] = NODE_ID(0, i+1);
      bndy_nodes[edge_idx] = node;
      edges_to_elems[edge_idx] = 2*i;
      which_edge[edge_idx++] = 2;
   }

   /* Top edge of mesh. */
   for (i = 0; i < edge_ct_x; i++)
   {
      node = NODE_ID(i, edge_ct_y);
      bndy_conn[conn_idx++] = node;
      bndy_conn[conn_idx++] = NODE_ID(i+1, edge_ct_y);
      bndy_nodes[edge_idx] = node;
      edges_to_elems[edge_idx] = 2*edge_ct_y*(i+1)-1;
      which_edge[edge_idx++] = 0;
   }

   /* Right edge of mesh. */
   for (i = edge_ct_y - 1; i >= 0; i--)
   {
      node = NODE_ID(edge_ct_x, i+1);
      bndy_conn[conn_idx++] = node;
      bndy_conn[conn_idx++] = NODE_ID(edge_ct_x, i);
      bndy_nodes[edge_idx] = node;
      edges_to_elems[edge_idx] = ele_ct-1-2*(edge_ct_y-1-i);
      which_edge[edge_idx++] = 2;
   }

   /* Bottom edge of mesh. */
   for (i = edge_ct_x - 1; i >= 0; i--)
   {
      node = NODE_ID(i+1, 0);
      bndy_conn[conn_idx++] = node;
      bndy_conn[conn_idx++] = NODE_ID(i, 0);
      bndy_nodes[edge_idx] = node;
      edges_to_elems[edge_idx] = edge_ct_y*2*i;
      which_edge[edge_idx++] = 0;
   }
...
read_base_space(void)
{
   ...
   int *mesh_conn, *bndy_conn,         /* Node to element and node to edge
                                        * connectivity data. */
       *bndy_nodes,                    /* Boundary to global node map. */
       *edges_to_elems,                /* Mapping from edges to elements. */
       *which_edge;                    /* Mapping from edge to edge of an
                                        * element. */
    ...
    */
   bndy_conn = (int*)malloc(edge_ct*2*sizeof(int));
   bndy_nodes = (int*)malloc(edge_ct*sizeof(int));
   edges_to_elems = (int*)malloc(edge_ct*sizeof(int));
   which_edge = (int*)malloc(edge_ct*sizeof(int));
   ...
   num = 0;
   saf_find_categories(SAF_ALL, db, SAF_UNIVERSE(db), "edges", SAF_ANY_ROLE,
                       SAF_ANY_TOPODIM, &num, &edges);
   if (num != 1)
   {
      FAILED;
      failCount++;
   ...
   if (strcmp(name, "edges") || !SAF_EQUIV(&role, SAF_TOPOLOGY) || tdim!=1)
   {
      FAILED;
      failCount++;
   }
   ...
   saf_find_matching_sets(SAF_ALL, db, "TOP_CELL", SAF_SPACE, 2,
                          SAF_EXTENDIBLE_FALSE, SAF_TOP_TRUE, &num,
                          &mesh);
   if (num != 1)
   {
      FAILED;
   ...
   num = 0;
   saf_find_matching_sets(SAF_ALL, db, "BOUNDARY", SAF_SPACE, 1,
                          SAF_EXTENDIBLE_FALSE, SAF_TOP_FALSE, &num,
                          &boundary);
   if (num != 1)
   {
      FAILED;
   ...
   num = 0;
   saf_describe_set(SAF_ALL, mesh, &p_name, &tdim, &srole, &extmode,
                    &topmode, &num, &coll_cats);
   ...
   num = 0;
   coll_cats = NULL;
   saf_describe_set(SAF_ALL, boundary, &p_name, &tdim, &srole, &extmode,
                    &topmode, &num, &coll_cats);
   if (strcmp(name, "BOUNDARY") || tdim != 1 ||
       srole != SAF_SPACE || extmode != SAF_EXTENDIBLE_FALSE ||
       topmode != SAF_TOP_FALSE || num != 2 ||
       ! (SAF_EQUIV(&coll_cats[0], nodes) ||
            SAF_EQUIV(&coll_cats[1], nodes)) ||
       ! (SAF_EQUIV(&coll_cats[0], edges) ||
            SAF_EQUIV(&coll_cats[1], edges)))
   {
      FAILED;
      failCount++;
   }
   ...
   saf_find_collections(SAF_ALL, mesh,     SAF_TOPOLOGY, SAF_CELLTYPE_POINT, 0,
                        SAF_DECOMP_FALSE, &num, &coll_cats);
   ...
   num = 0;
   coll_cats = NULL;
   saf_find_collections(SAF_ALL, mesh,     SAF_TOPOLOGY, SAF_CELLTYPE_TRI,   2,
                        SAF_DECOMP_TRUE, &num, &coll_cats);
   ...
   num = 0;
   coll_cats = NULL;
   saf_find_collections(SAF_ALL, boundary, SAF_TOPOLOGY, SAF_CELLTYPE_POINT, 0,
                        SAF_DECOMP_FALSE, &num, &coll_cats);
   if (num != 1 || ! SAF_EQUIV(&coll_cats[0], nodes))
   {
      FAILED;
   ...
   num = 0;
   coll_cats = NULL;
   saf_find_collections(SAF_ALL, boundary, SAF_TOPOLOGY, SAF_CELLTYPE_LINE,  1,
                        SAF_DECOMP_TRUE, &num, &coll_cats);
   ...
   saf_describe_collection(SAF_ALL, boundary, nodes, &cell_type, &num, NULL,
                           &is_decomp, NULL);
   if (cell_type != SAF_CELLTYPE_POINT || num != edge_ct ||
       is_decomp != SAF_DECOMP_FALSE)
   {
      FAILED;
      failCount++;
   ...
   saf_describe_collection(SAF_ALL, boundary, edges, &cell_type, &num, NULL,
                           &is_decomp, NULL);
   if (cell_type != SAF_CELLTYPE_LINE || num != edge_ct || is_decomp != SAF_DECOMP_TRUE)
   {
      FAILED;
      failCount++;
   }
   ...
   num = 0;
   saf_find_subset_relations(SAF_ALL, db, mesh, boundary, SAF_COMMON(nodes),
                             &num, &srel1);
   if (num != 1)
   {
      FAILED;
   ...
   num = 0;
   saf_find_subset_relations(SAF_ALL, db, mesh, boundary,
                             SAF_BOUNDARY(elems, edges), &num, &srel2);
   if (num != 1)
   {
      FAILED;
      failCount++;
   ...
   saf_describe_subset_relation(SAF_ALL, srel1, &sup_set, &sub_set, &sup_cat,
                                &sub_cat, &sbmode, &cbmode, &srtype,
                                &data_type);
   ...
   /* In the case of a boundary subset relation, two buffers of data are needed
    * to describe the relation.  However, only one data_type is returned here.
    * It is not clear from examining the code which buffer's data_type is being
    * described here.  There is no requirement that the types of these buffers
    * be the same and both buffers are written to the dataset.  Therefore, it
    * is essential that the types be identified properly prior to reading the
    * the data from the database.  This describe is insufficient.  It and its
    * companion for the topological relations must be changed so that they
    * describe both buffers' types.  Note that saf_get_... does provide the
    * proper interface but it, too, does the wrong thing and assumes that both
    * buffers must be of the same type and therefore may not give the proper
    * result.
    */
   saf_describe_subset_relation(SAF_ALL, srel2, &sup_set, &sub_set, &sup_cat,
                                &sub_cat, &sbmode, &cbmode, &srtype,
                                &data_type);
   if (!SAF_EQUIV(&sup_set, mesh) || !SAF_EQUIV(&sub_set, boundary) ||
       !SAF_EQUIV(&sup_cat, elems) || !SAF_EQUIV(&sub_cat, edges) ||
       sbmode != SAF_BOUNDARY_TRUE || cbmode != SAF_BOUNDARY_TRUE ||
       !SAF_EQUIV(&srtype, SAF_TUPLES) || !H5Tequal(data_type, H5T_NATIVE_INT) )
   {
      FAILED;
      failCount++;
   ...
   saf_get_count_and_type_for_subset_relation(SAF_ALL, srel1, NULL, &a_count,
                                              &a_type, &b_count, &b_type);
   if (!H5Tequal(a_type, H5T_NATIVE_INT) || b_type>0 ||
       a_count != (size_t)edge_ct || b_count != 0)
   {
      FAILED;
      failCount++;
   }
   else
   {
      ...
      {
         /* We got the buffer--verify that the contents are as expected. */
   ...
   /* In the case of boundary subset relations, the results from this may be
    * misleading.  There is no requirement that the two buffers have the same
    * type.  However, the type of one of the buffers (its not clear which one)
    * is returned as the type of both by this function.  See comment above for
    * saf_describe_subset_relation.  Use with caution.
    */
   saf_get_count_and_type_for_subset_relation(SAF_ALL, srel2, NULL, &a_count,
                                              &a_type, &b_count, &b_type);
   if (!H5Tequal(a_type, H5T_NATIVE_INT) || !H5Tequal(b_type, H5T_NATIVE_INT) || a_count != (size_t) edge_ct ||
       b_count != (size_t)edge_ct)
   {
      FAILED;
      failCount++;
   }
    ...
    */
   saf_find_topo_relations(SAF_ALL, db, mesh, NULL, &num, &trel1);
   if (num != 1)
   {
      FAILED;
      failCount++;
   ...
   num = 0;
   saf_find_topo_relations(SAF_ALL, db, boundary, NULL, &num, &trel2);
   if (num != 1)
   {
      FAILED;
      failCount++;
   ...
   saf_describe_topo_relation(SAF_ALL, trel1, &containing_set, &the_pieces,
   ...
   if (!SAF_EQUIV(&containing_set, mesh) ||
       ...
       !_saf_is_self_decomp(&storage_decomp) || !SAF_EQUIV(&trtype, SAF_UNSTRUCTURED) ||
   ...
   saf_describe_topo_relation(SAF_ALL, trel2, &containing_set, &the_pieces,
                              &range_s, &range_c, &storage_decomp, &trtype,
                              &data_type);
   if (!SAF_EQUIV(&containing_set, boundary) ||
       !SAF_EQUIV(&the_pieces, edges) || !SAF_EQUIV(&range_s, mesh) ||
       !SAF_EQUIV(&range_c, nodes) ||
       !_saf_is_self_decomp(&storage_decomp) || !SAF_EQUIV(&trtype, SAF_UNSTRUCTURED) ||
       !H5Tequal(data_type, H5T_NATIVE_INT))
   {
   ...
   saf_get_count_and_type_for_topo_relation(SAF_ALL, trel1, NULL, NULL, &a_count, &a_type,
                                            &b_count, &b_type);
   ...
   saf_get_count_and_type_for_topo_relation(SAF_ALL, trel2, NULL, NULL, &a_count, &a_type,
                                            &b_count, &b_type);
   if (!H5Tequal(a_type, H5T_NATIVE_INT) || !H5Tequal(b_type, H5T_NATIVE_INT) ||
       a_count != 1 || b_count != (size_t)edge_ct*2)
   {
      FAILED;
      failCount++;
   }
...
read_coord_field(void)
{
   ...
   num = 0;
   saf_describe_field_tmpl(SAF_ALL, coords_ctmpl, &p_name, &atype,
                           &basis, NULL, &num, NULL);
   if (strcmp(name, "e2_on_triangle_mesh_ctmpl") ||
   ...
   num = 0;
   saf_describe_field_tmpl(SAF_ALL, coords_ftmpl, &p_name, &atype,
                           &basis, NULL, &num, NULL);
   if (strcmp(name, "e2_on_triangle_mesh_tmpl") ||
       !SAF_EQUIV(&atype, SAF_ALGTYPE_VECTOR) ||
...
{
   /* Creates the scalar field dofs on the boundary of a rectangular triangle
    * mesh.
    * edge_ct_x = number of edges in x-dir
    * edge_ct_y = number of edgs in y-dir
    *
   ...
   /* allocate dofs as single index array,
    * but treat as dofs[nx+1][ny+1][2]; */
   dofs = (double*)malloc(2*(edge_ct_x+edge_ct_y)*sizeof(double));
   ...
   /* Top boundary of mesh. */
   for(i = 0; i < edge_ct_x; i++)
   {
      x = i*delx;
      dofs[edge_ct_y+i] = x*x+1;
   }
   ...
   {
      y = (edge_ct_y-i)*dely;
      dofs[edge_ct_x+edge_ct_y+i] = 1+y*y;
   }
   ...
   {
      x = (edge_ct_x-i)*delx;
      dofs[edge_ct_x+2*edge_ct_y+i] = x*x;
   }
...
read_scalar_field(void)
{
   ...
   saf_find_field_tmpls(SAF_ALL, db,
                        "at0_on_triangle_mesh_boundary_tmpl", SAF_ALGTYPE_SCALAR,
   ...
   saf_describe_field_tmpl(SAF_ALL, scalar_ftmpl, &p_name, &atype,
                           &basis, NULL, &num, NULL);
   if (strcmp(name, "at0_on_triangle_mesh_boundary_tmpl") ||
       !SAF_EQUIV(&atype, SAF_ALGTYPE_SCALAR) ||
       !SAF_EQUIV(&basis, SAF_UNITY) || num != 1)
   {
      FAILED;
   ...
   num = 0;
   saf_find_fields(SAF_ALL, db, boundary, "scalar field", SAF_ANY_QUANTITY,
   ...
   if (count != 2*(size_t)(edge_ct_x+edge_ct_y) || !H5Tequal(dof_type, H5T_NATIVE_DOUBLE))
   {
      FAILED;
      failCount++;
   }
...
read_stress_field(void)
{
   ...
   saf_describe_field_tmpl(SAF_ALL, stress_ctmpl, &p_name, &atype,
                           &basis, NULL, &num, NULL);
   if (strcmp(name, "st2_e2_on_triangle_mesh_ctmpl") ||
       !SAF_EQUIV(&atype, SAF_ALGTYPE_SCALAR) ||
       !SAF_EQUIV(&basis, SAF_UNITY) || num != 1)
   {
      FAILED;
   ...
   saf_describe_field_tmpl(SAF_ALL, stress_ftmpl, &p_name, &atype,
                           &basis, NULL, &num, NULL);
   if (strcmp(name, "st2_e2_on_triangle_mesh_tmpl") ||
