 ...
 * Audience:    Public
 * Chapter:    	Tests
 * Purpose:    	Test of creation of a triangle mesh with an identified boundary
 *              and fields defined on them.
 *
 * Description: This test demonstrates the creation of a simple 2D triangular
 *              mesh immersed in 2D Euclidean vector space.  The boundary of
 ...
 *              boundary.  The global coordinate and scalar fields have their
 *              dofs defined on nodes while the tensor field's dofs are defined
 *              on the elements.  The program accepts one optional argument,
...
SAF_Cat nodes, elems,     /* Handles to the nodes, elements and edges */
        edges;            /* categories. */
SAF_Set mesh, boundary;   /* Handles to the mesh and boundary set. */
...
int edge_ct_x, edge_ct_y, /* The number of triangle edges in
    ...
    node_ct, ele_ct,      /* The number of nodes and elements int the mesh. */
    edge_ct;              /* The number of edges in the boundary. */
...
make_base_space(void)
{
   ...
   SAF_Rel srel, trel;                 /* Handles to the subset and topological
                                        * relations. */
   int nodes_per_mesh_element[] = {3}, /* Number of nodes for each element */
       nodes_per_bndy_element[] = {2}, /* in the mesh and boundary. */
       *mesh_conn, *bndy_conn,         /* Node to element and node to edge
                                        * connectivity data. */
   ...
   saf_declare_set(SAF_ALL, db, "BOUNDARY", 1, SAF_SPACE,
   ...
   saf_declare_collection(SAF_ALL, &boundary, &nodes,  SAF_CELLTYPE_POINT, edge_ct,
                          SAF_1DC(edge_ct), SAF_DECOMP_FALSE);
   saf_declare_collection(SAF_ALL, &boundary, &edges,  SAF_CELLTYPE_LINE,  edge_ct,
                          SAF_1DC(edge_ct),  SAF_DECOMP_TRUE);
   PASSED;
   ...
   saf_declare_subset_relation(SAF_ALL,db, &mesh, &boundary,
                               ...
                               SAF_INT, bndy_nodes,  H5T_NATIVE_INT,    NULL,
                               &srel);
   saf_write_subset_relation(SAF_ALL, &srel, H5T_NATIVE_INT, NULL, H5T_NATIVE_INT, NULL, saf_file);

   /* Edges of the boundary in elements of the whole. */
   saf_declare_subset_relation(SAF_ALL, db, &mesh, &boundary,
                               SAF_BOUNDARY(&elems, &edges), SAF_TUPLES,
                               SAF_INT, edges_to_elems, H5T_NATIVE_INT, which_edge,
                               &srel);
   saf_write_subset_relation(SAF_ALL, &srel, H5T_NATIVE_INT, NULL, H5T_NATIVE_INT, NULL, saf_file);
   PASSED;
   ...
   saf_write_topo_relation(SAF_ALL, &trel, H5T_NATIVE_INT, nodes_per_mesh_element,
                           H5T_NATIVE_INT, mesh_conn, saf_file);
   ...
   saf_declare_topo_relation(SAF_ALL,db, &boundary, &edges, &mesh, &nodes,
                             SAF_SELF(db), &mesh, SAF_UNSTRUCTURED, H5I_INVALID_HID, NULL,
                             H5I_INVALID_HID, NULL, &trel);
   saf_write_topo_relation(SAF_ALL, &trel, H5T_NATIVE_INT, nodes_per_bndy_element,
                           H5T_NATIVE_INT, bndy_conn, saf_file);
   PASSED;
...
{
   /* Construct the scalar field on the boundary. */
   ...
   saf_declare_field_tmpl(SAF_ALL,db, "at0_on_triangle_mesh_boundary_tmpl",
                          SAF_ALGTYPE_SCALAR, SAF_UNITY, SAF_QLENGTH, 1,
                          NULL, &scalar_ftmpl);
   PASSED;
