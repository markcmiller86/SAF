...
class StateData extends JPanel {

	Float float_value;

	public StateData( int set_id, int coll_id,  int field_id ) {
		...
		int data_size;
		int coll_count;
		int fieldtmpl_id, dep_var_fieldtmpl_id;
		int state_count;
		JButton rowButton;
		String data_string="";
		...
		category_name = saf_wrap.get_coll_cat_name( set_id, coll_id );
		...
		// state_count = saf_wrap.get_field_data_size( field_id );
		state_count = saf_wrap.get_state_num_states( field_id );
		assoc_ratio = saf_wrap.get_field_assoc_ratio( field_id );


		num_comps = saf_wrap.get_fieldtmpl_num_comp( fieldtmpl_id );
		dep_var_fieldtmpl_id = saf_wrap.get_fieldtmpl_component( fieldtmpl_id, 1);
		num_comps = saf_wrap.get_fieldtmpl_num_comp( dep_var_fieldtmpl_id );
		...
		String columnzeroheader = "Fields in State: " + state_name;
		Object [] column_headers = new Object[state_count+1];
		column_headers[0] = columnzeroheader;

		for( i = 1 ; i <= state_count; i++ ) {
			column_headers[i] = "state " + i;
		} 

		Object [][] tabledata = new Object[num_comps][state_count+1];
		...
		for( i = 0; i < num_comps ; i++ ) {
			...
			comp_field_id = saf_wrap.get_state_data( field_id, 0, i);

			comp_field_name = saf_wrap.get_field_name( comp_field_id );

			tabledata[i][0] = comp_field_name;
...
*/
		...
		for( i = 0 ; i < state_count; i++ ) {
			for( j = 0 ; j < num_comps ; j++ ) {
				column_num = i+1;
				comp_field_id = saf_wrap.get_state_data( field_id, i, j);
				Object [] statecell = { "state " + column_num + ": " + tabledata[j][0], new Integer(comp_field_id) };
				tabledata[j][i+1] = statecell;
		...
		for( i = 1; i <= state_count; i++ ) {
			table.setDefaultRenderer( table.getColumnClass(i), buttoncellrenderer );
			table.setDefaultEditor( table.getColumnClass(i), buttoncelleditor );
			table.getColumn((String)column_headers[i]).setMinWidth(200);
		}
