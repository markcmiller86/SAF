     ...
     char **argv)
{
...

  {
    ...
    float *index_value;
    int num_indexes, in, sn, fn,  num_fields_in_state;
    int nc, ns;
	...
	for( sn = 1 ; sn < suites[i]->num_fields ; sn++ ) {
	  ...
	  for( fn = 0 ; fn < num_fields_in_state ; fn++ ) {
	    ...
	    for( nc = 0 ; nc < base_space->num_colls; nc++ ) {
		  ...
		  (!strcmp(base_space->collections[nc].cat_name,"procs"))  ) {
