...
|
|               void except_uncaught ( X,filename,line )
|
|                   char *X;
|                   char *filename;
|                   int   line;
|
|           This private function catches uncaught exceptions.  By default
|           this function simply complains and quits.  By properly exploiting
|           the fact that this function is a object module in a library
|           users can provided their own version of this function.
|
+--------------------------------------------------------------------------- - -
|
|       Author:
...
extern void except_uncaught ( char *X, char *filename, int line )
{  fprintf(stderr,"except: uncaught exception ");
   if (X != 0)
      fprintf(stderr,"\"%s\" ",X);
